    ...
    for epoch in range(epochs):
        ...
        for rgbd_batch, grasps_batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs}"):
            ...
            for i in range(len(preds)):
                gt = torch.tensor(grasps_batch[i], device=preds.device)  # (N_grasps, 5)
        ...
        with torch.no_grad():
            for rgbd_batch, grasps_batch in val_loader:
                ...
                batch_loss = 0.0
                for i in range(len(preds)):
                    gt = torch.tensor(grasps_batch[i], device=preds.device)
...
def main(mode="train"):
    ...
    train_dataset = JacquardDataset(train_folders, base_path, transform=transform)
    val_dataset = JacquardDataset(val_folders, base_path, transform=transform)
    ...
    if mode == "verify":
        ...
        torch.save(model.state_dict(), "grasp_rgbd_mask_perfect.pth")
    elif mode == "predict":
