...
from GraspEstimatorOnlyWidthAngle import GraspPredictorNet, draw_grasp_on_rgb
import math

# --- Configuration ---
ROOT_DIR = "trashResults/masks"
MODEL_PATH = "grasp_rgbd_mask_finetuned_stereo_augmented_WAOnly.pth"
NUM_SAMPLES = 55
...
for i in range(1, NUM_SAMPLES + 1):
    ...
    # Replace x and y with mask center
    mask_np = rgbd_tensor[0, 4].cpu().numpy()  # Mask channel
    ys, xs = np.where(mask_np > 0.5)
    if len(xs) == 0 or len(ys) == 0:
        print(f"[SKIP] Empty mask in sample {i}")
        continue
    center_x = np.mean(xs)
    center_y = np.mean(ys)

    # Construct the final grasp prediction with injected center
    final_pred = np.array([center_x, center_y, pred[0], pred[1], 0.9])  # dummy quality

    # Visualization and logging
    rgb_tensor = rgbd_tensor[0, :3, :, :]
    draw_colored_grasps(rgb_tensor, final_pred)

    print(f"Sample {i} - Predicted grasp:")
    print(f"  x={center_x:.2f}, y={center_y:.2f}, angle={pred[0]:.2f}, width={pred[1]:.2f}")
